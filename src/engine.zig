...
test "Engine basic creation" {
    const Item = enum { Flour, Bread };
    const TestHooks = struct {};

    var engine = Engine(u32, Item, TestHooks).init(std.testing.allocator, .{});
    defer engine.deinit();
...
test "Engine item_added updates storage state" {
    const Item = enum { Flour, Bread };
    const TestHooks = struct {};
    ...
    const result = engine.itemAdded(1, .Flour);
    try std.testing.expect(result);
    try std.testing.expect(engine.getStorageHasItem(1) == true);
    try std.testing.expect(engine.getStorageItemType(1) == .Flour);
}

test "Engine worker_available triggers assignment" {
    const Item = enum { Flour, Bread };

    var assigned_worker: ?u32 = null;
    var assigned_ws: ?u32 = null;

    const TestHooks = struct {
        assigned_worker_ptr: *?u32,
        assigned_ws_ptr: *?u32,

        pub fn worker_assigned(self: *@This(), payload: anytype) void {
            self.assigned_worker_ptr.* = payload.worker_id;
            self.assigned_ws_ptr.* = payload.workstation_id;
        }
    };

    var engine = Engine(u32, Item, TestHooks).init(
        std.testing.allocator,
        .{ .assigned_worker_ptr = &assigned_worker, .assigned_ws_ptr = &assigned_ws },
    );
    defer engine.deinit();

    // Setup: storage with item, empty output, workstation, worker
    try engine.addStorage(1, .Flour); // EIS with flour
    try engine.addStorage(2, null); // IIS empty
    try engine.addStorage(3, null); // IOS empty
    try engine.addStorage(4, null); // EOS empty
    ...
    // Worker becomes available - should trigger assignment
    _ = engine.workerAvailable(10);

    try std.testing.expect(assigned_worker == 10);
    try std.testing.expect(assigned_ws == 100);

...
// Tests
test "TaskHookPayload creation" {
    const Payload = TaskHookPayload(u32, enum { Flour, Bread });
...
test "GameHookPayload creation" {
    const Payload = GameHookPayload(u32, enum { Flour, Bread });
...
test "HookDispatcher calls hook method" {
    ...
    var dispatcher = HookDispatcher(u32, Item, TestHooks).init(hooks);
...
test "HookDispatcher ignores missing hooks" {
    const Item = enum { Flour, Bread };

    const EmptyHooks = struct {};

    const hooks = EmptyHooks{};
    var dispatcher = HookDispatcher(u32, Item, EmptyHooks).init(hooks);
